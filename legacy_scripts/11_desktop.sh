...
setup_power_management() {
  ...
  # .tlp.conf is now managed by chezmoi and should be at $HOME/.tlp.conf
  if [[ -f "$HOME/.tlp.conf" ]]; then
    sudo ln -sf "$HOME/.tlp.conf" /etc/tlp.conf
  else
    echo "Warning: $HOME/.tlp.conf not found. Make sure chezmoi has been applied."
  fi
}

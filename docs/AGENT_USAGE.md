...
Each is run as its own `ccusage claude|codex daily --json` with `CLAUDE_CONFIG_DIR` / `CODEX_HOME`
set explicitly. This matters when running `ccusage` by hand:
...
`total`, `cost`; `model` rows add `model` and `estimated`.
...
- **Upsert rules:** a re-run replaces the rows for each `(profile, date)` it sees; it **never deletes**
  a day ccusage no longer returns (that day was pruned at the source); and it **never lets a day's total
  shrink**, so a partially-pruned day cannot overwrite a complete one.
...
re-check `NORMALISE_JQ` in the script if it fails.
...
- **Timezone is pinned to `Asia/Tokyo`** in the script so archived day boundaries never shift.
